...
use actix_web::{
    dev::AppService, get, post, web, web::Redirect, App, HttpResponse, HttpServer, Responder,
};
...
pub async fn register_new_user(
    ...
    let count = sqlx::query("SELECT * FROM users WHERE email = $1")
        .bind(&new_user.email)
        .fetch_all(&app_state.pool)
        .await
        .unwrap()
    ...
    if count > 0 {
        return HttpResponse::BadRequest().json(Response {
            message: "User with email already exists.".to_string(),
        });
    }

    let hashed_password = bcrypt::hash(&new_user.password, bcrypt::DEFAULT_COST);
    let max_existing_id = sqlx::query_scalar::<_, i32>("SELECT MAX(user_id) FROM users")
        .fetch_one(&app_state.pool)
        .await
        .unwrap();

    let user = User {
        user_id: max_existing_id + 1,
        name: new_user.name.clone(),
        email: new_user.email.clone(),
        HashedPassword: hashed_password.unwrap(),
        Elo: 1000,
        Age: new_user.age,
    };
    ...
    // save to database
    let query = format!(
        "INSERT INTO users (name, email, HashedPassword, Elo, Age) VALUES ('{}', '{}', '{}', {}, {})",
        user.name, user.email, user.HashedPassword, user.Elo, user.Age
    );
    let insert_user = sqlx::query(&query).execute(&app_state.pool).await.unwrap();
...
async fn update_users_elo(
    path: web::Path<(i32, i32)>,
    app_state: web::Data<AppState>,
) -> impl Responder {
    let (user1, user2) = path.into_inner();
    //I know there is a more efficient way to do this but I just want it to work.
    let eloUser1: Option<i32> = sqlx::query_scalar("SELECT Elo FROM users WHERE user_id = $1")
        .bind(&user1)
        .fetch_optional(&app_state.pool)
        .await
        .unwrap();

    let eloUser2: Option<i32> = sqlx::query_scalar("SELECT Elo FROM users WHERE user_id = $1")
        .bind(&user2)
        .fetch_optional(&app_state.pool)
        .await
        .unwrap();
    ...
    let update_userW = sqlx::query(&query).execute(&app_state.pool).await.unwrap();

    let query2 = format!(
        "UPDATE users SET Elo = {} WHERE user_id = {}",
        updated_elo_2, user2
    );
    let update_userL = sqlx::query(&query2).execute(&app_state.pool).await.unwrap();
